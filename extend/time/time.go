...
func NewTime(tt time.Time, setGlobal bool) STime {
	ct := STime{
		Time: tt,
	}
...
func NewSecond(second int64) STime {
	return NewTime(time.Unix(second, 0), true)
}

func NewMillisecond(millisecond int64) STime {
...
func CreateFromTimestamp(timestamp int64) STime {
	ts := timestamp
...
func CreateFromDateTime(year int, month int, day int, hour int, minute int, second int) STime {
	t := time.Date(year, time.Month(month), day, hour, minute, second, 0, offsetLocation)
	return NewTime(t, false)
}

// CreateFromDate 从年月日创建 Carbon 实例(默认时区)
func CreateFromDate(year int, month int, day int) STime {
	now := Now()
...
func CreateFromTime(hour int, minute int, second int) STime {
	now := Now()
...
func ParseByDuration(duration string) (time.Duration, error) {
	...
	if err != nil {
		err = serror.Errorf("invalid duration %s", duration)
	}
	return td, err
}
