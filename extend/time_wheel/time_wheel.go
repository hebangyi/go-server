...
import (
	clog "go-server/extend/log"
	sTime "go-server/extend/time"
	cutils "go-server/extend/utils"
	"sync/atomic"
	"time"
	"unsafe"
)
...
func (tw *TimeWheel) add(t *TimerTask) bool {
...
func (tw *TimeWheel) addOrRun(t *TimerTask) {
	if !tw.add(t) {
		// Already expired
...
// It returns a TimerTask that can be used to cancel the call using its Stop method.
func (tw *TimeWheel) AfterFunc(id uint64, d time.Duration, f func(), async ...bool) *TimerTask {
	t := &TimerTask{
		id:         id,
		expiration: TimeToMS(sTime.NewTime(sTime.Now().Add(d), true)),
		task:       f,
...
func (tw *TimeWheel) AddEveryFunc(id uint64, d time.Duration, f func(), async ...bool) *TimerTask {
	return tw.ScheduleFunc(id, &EverySchedule{Interval: d}, f, async...)
}

func (tw *TimeWheel) BuildAfterFunc(d time.Duration, f func()) *TimerTask {
	id := NextId()
	return tw.AfterFunc(id, d, f)
}

func (tw *TimeWheel) BuildEveryFunc(d time.Duration, f func(), async ...bool) *TimerTask {
	id := NextId()
	return tw.AddEveryFunc(id, d, f, async...)
}

// ScheduleFunc calls f (in its own goroutine) according to the execution
// plan scheduled by s. It returns a TimerTask that can be used to cancel the
...
// is non-zero.
func (tw *TimeWheel) ScheduleFunc(id uint64, s Scheduler, f func(), async ...bool) *TimerTask {
	expiration := s.Next(sTime.Now())
