...
import (
	sstring "go-server/extend/string"
	sutils "go-server/extend/utils"
	"math/rand"
	"reflect"
	"strings"
	"time"
...
func StringToInt(strSlice []string) []int {
	...
	for _, s := range strSlice {
		if sutils.IsNumeric(s) {
			val, ok := sstring.ToInt(s)
			if ok {
